...
<div class="col-md-3">
    <?= $form->field($model, 'company_name')->textInput(['readonly'=> true,'maxlength' => true,'class'=>'form-control input-sm typehead','onkeyup'=>'EmptyEsc(this.value,event);','value'=>$delivery['company_name']])->label('COMPANY NAME') ?>
 
    <input type="hidden" id="custId" name="custId" value="<?php echo $delivery['cust_id'];?>">

     <input type="hidden" id="convertdc" name="CONVERTDC" value="<?php echo $delivery['id']; ?>">

     <?= $form->field($model, 'customer_name')->hiddenInput(['readonly' => true,'value'=>$delivery['cust_name']])->label(false) ?>

</div>

<div class="col-md-2">
    <?= $form->field($model, 'gstin_no')->textInput(['readonly'=>true,'value'=>$delivery['gstin_no']])->label('GSTIN NO') ?>
</div>
<div class="col-md-2">
            <?= $form->field($model, 'tax_type')->dropDownList(['GST'=>'GST','IGST'=>'IGST'],['options' => $delivery['convert_status'] == '' ? [$delivery['gst_type'] => ['Selected'=>'selected']] : $invoice_table['tax_type'],'class' => 'form-control input-sm gst_percent','id'=>'gst_percent','prompt'=>"SELECT"])->label('GST Type') ?>
        </div>
<div class="col-md-1">
            <?= $form->field($model, 'tax_id')->dropDownList($tax_master,['options' => $delivery['convert_status'] == '' ? [$delivery['gst_id'] => ['Selected'=>'selected']] : $invoice_table['tax_id'],'class' => 'form-control no-padding input-sm gst_percent','id'=>'gst_percent'])->label('GST (%)') ?>
        </div>
...
?>
    ...
    <td><input type="text" class="form-control input-sm description_table disabled_field"  style="text-align:left;" required id="description_table<?php echo $k; ?>" value="<?php echo $value['description']; ?>" name="InvoiceRefTbl[description_table][]"></td>

    <td><input type="text" class="form-control input-sm amount_table disabled_field decimal_validated"  style="text-align:right;" readonly id="amount_table<?php echo $k; ?>" value="<?php echo $value['amount']; ?>" name="InvoiceRefTbl[amount_table][]"></td>
    ...
    <td style="text-align:center;"><button type="button" class="btn btn-danger btn-xs disabled_field" style="text-align:center;" onclick="Remove('<?php echo $k; ?>');">Remove</button></td></tr>
              ...
              <?php  $k++; } } else if($delivery['convert_status'] == '') {  
  
   

    foreach ($delivery_ref as $key => $value) {
            ...
            <tfoot id="tfoot_total_amount">
                <tr>
                    ...
                    <td style="text-align: right;color: blue;" id="total_amount_add">
                        <?php  if($delivery['convert_status'] == '')
                            {  ?>     

                           <?php echo  $delivery['tot_amt']; ?>
                        <?php   } else if($delivery['convert_status'] == 'IV')
                                    {
                                ?>
...
<hr style="width:100%;border-top:2px solid #4682b4;opacity:10;">
 ...
 <div class="pull-right">
        ...
        <?= Html::submitButton('<i class="fa fa-eye"></i> Preview', ['class' => 'btn btn-danger','id'=>'preview_button','value'=>'preview']) ?>
         ...
         <?= Html::submitButton($delivery['convert_status'] == '' ? '<i class="fa fa-fw fa-print"></i>Save & Print' : '<i class="fa fa-fw fa-print "></i>Update & Print', ['value'=>'yes','id'=>$delivery['convert_status'] == '' ? 'saved_print' : 'updated_print' ,'class' => $delivery['convert_status'] == '' ? 'btn btn-success tax' : 'btn btn-success ']) ?>
        
        <?= Html::submitButton($delivery['convert_status'] == '' ? '<i class="fa fa-fw fa-save"></i>Save' : '<i class="fa fa-fw fa-edit "></i>Update', ['value'=>'no','id'=>$delivery['convert_status'] == '' ? 'saved' : 'updated' ,'class' => $delivery['convert_status'] == '' ? 'btn btn-primary tax' : 'btn btn-primary updatetax']) ?>
...
<?php 
    ...
    ?>
       ...
       var print_url='<?php echo Yii::$app->homeUrl ?>?r=invoice-table/update&id='+'<?php echo $delivery['convert_id']; ?>';
       var save_url='<?php echo Yii::$app->homeUrl ?>?r=invoice-table/updateprint&id='+'<?php echo $delivery['convert_id']; ?>';
...
function Add_To_Grid()
{
   ...
   /* if(sac_code === null || sac_code === '')
    ...
    '<td><input type="text" class="form-control input-sm description_table disabled_field"  style="text-align:left;" required id="description_table'+current+'" value="'+description+'" name="InvoiceRefTbl[description_table][]"></td>'+

    '<td><input type="text" class="form-control input-sm amount_table disabled_field decimal_validated"  style="text-align:right;" readonly id="amount_table'+current+'" value="'+amount+'" name="InvoiceRefTbl[amount_table][]"></td>'+
