...
<div class="panel-body" style="height:1000px;">
	...
	<div class="col-md-12">				
		...
		<div class="col-md-2">		
			<?= $form->field($subvisit, 'sub_visit')->textInput(['class' => 'form-control number','name'=>'subVisit_sub_visit','onkeyup'=>'SubVisit(event,this);','required'=>true])->label('Sub Visit No') ?>
		</div>
		<div class="col-md-2">		
			<?= $form->field($subvisit, 'mr_number')->textInput(['class' => 'form-control number','readonly'=>true,'name'=>'subVisit_mr_number','required'=>true])->label('MR Number') ?>
		</div>
	...
	<div class="col-md-12">
		<div class="col-md-12">		
			<?= $form->field($newpatient, 'patientname')->textInput(['class' => 'form-control','readOnly'=>true,'name'=>'newpatient_patientname','required'=>true])->label('Patient Name') ?>
		</div>
		<div class="col-md-12">		
			<?= $form->field($newpatient, 'pat_age')->textInput(['class' => 'form-control','readOnly'=>true,'name'=>'newpatient_pat_age','required'=>true])->label('Age') ?>
		</div>	
		<div class="col-md-12">		
			<?= $form->field($newpatient, 'pat_sex')->dropDownList(['Male' => 'Male','Female' => 'Female'], ['class' => 'form-control','name'=>'newpatient_pat_sex','readOnly'=>true,'required'=>true])->label('Gender') ?>
		</div>
		
		<div class="col-md-12">		
			<?= $form->field($subvisit, 'consultant_doctor')->dropDownList($physicianmaster, ['prompt'=>'--SELECT--','class' => 'form-control','readOnly'=>true,'name'=>'subVisit_consultant_doctor','required'=>true])->label('Doctor Name') ?>
		</div>				
		<div class="col-md-12">		
			<?= $form->field($cancellog, 'opd_type')->dropDownList(['O' => 'Private OPD'], ['class' => '  form-control','name'=>'cancelLogTable_opd_type','required'=>true])->label('OPD Type') ?>
		</div>
		<div class="col-md-12">		
			<?= $form->field($cancellog, 'towards')->textInput(['class' => 'form-control','value'=> 'Towards OP Visit Refund','readOnly'=>true,'required'=>true])->label('Towards') ?>
		</div>	
		<div class="col-md-12">		
			<?= $form->field($cancellog, 'refund_type')->dropDownList(['C' => 'Cancelled'], ['class' => 'form-control','readOnly'=>true,'required'=>true])->label('Refund Type') ?>
		</div>
		<div class="col-sm-12">		
			<?= $form->field($subvisit, 'pay_mode')->dropDownList($paymenttype, ['class' => 'form-control','readOnly'=>true,'required'=>true])->label('Payment Mode') ?>
		</div>							
	</div>
	</div>
		</div>
	...
	<div class="col-md-12">				
		...
		<div class="col-sm-12">		
			<?= $form->field($subvisit, 'net_amt')->textInput(['class' => 'form-control number','name'=>'subVisit_net_amt','readOnly'=>true,'required'=>true])->label('Consultant Amount') ?>
		</div>	
		<div class="col-sm-12">		
			<?= $form->field($cancellog, 'cancel_amt')->textInput(['class' => 'form-control number','onkeyup'=>'AmtInWords(this.value);','name'=>'cancelLogTable_cancel_amt','required'=>true,'readOnly'=>true])->label('Cancel Amount') ?>
		</div>
		<div class="col-sm-12">		
			<?= $form->field($cancellog, 'amt_words')->textInput(['class' => 'form-control','readOnly'=>true,'name'=>'cancelLogTable_amt_words','required'=>true])->label('Amount In Words') ?>
		</div>	
		<div class="col-sm-12">		
			<?= $form->field($cancellog, 'paid')->textInput(['class' => 'form-control number','onkeyup'=>'PaidAmount(this.value);','name'=>'cancelLogTable_paid','required'=>true,'readOnly'=>true])->label('Paid') ?>
		</div>
		<div class="col-sm-12">		
			<?= $form->field($cancellog, 'reason_cancelled')->textArea(['class' => 'form-control','name'=>'cancelLogTable_reason_cancelled','required'=>true])->label('Reason For Cancellation') ?>
		</div>							
	</div>
	</div>	
		</div>
	 ...
	 <?= Html::submitButton('Save', ['class' => 'summited btn btn-success','onclick'=>'SaveRegisterForm();']) ?>
      ...
      <div class="modal-body">
        ...
        <div class="" id="patient_history_report">
				...
				<tbody id='set_patient_data'>
				 	<?php if(!empty($subvisit_datatable)) 
				 		 {
					...
					?>
								<tr id='<?php echo $value['sub_id'];?>'>
									...
									<td><?php echo $newpatient_datatable[$value['pat_id']]['patientname'];?></td>
									<td><?php echo $newpatient_datatable[$value['pat_id']]['pat_mobileno'];?></td>
									<td><?php echo $physicianmaster_datatable[$value['consultant_doctor']];?></td>
									<td><button type="button" class='btn btn-xs btn-success select_sub_visit' onclick='SubVisitFetch(<?php echo $value['sub_id'];?>);' data-id='<?php echo $value['sub_id'];?>'>Select</button></td>
								</tr>
					<?php } }?>	
				</tbody>
				</table>
...
$(document).ready(function() {
 ...
 function SubVisitFetch(subvisitid) 
 {
   	if(subvisitid != '')
 	{
 		$.ajax({
        type: "POST",
        url: "<?php echo Yii::$app->homeUrl . "?r=cancel-log-table/fetchdetails&id=";?>"+subvisitid,
        success: function (result) 
        { 
        	var obj = $.parseJSON(result);
        	$('#subvisit-sub_visit').val(obj[0]);
        	$('#subvisit-mr_number').val(obj[1]);
        	$('#newpatient-patientname').val(obj[2]);
        	$('#newpatient-pat_age').val(obj[3]);
        	$('#newpatient-pat_sex').html(obj[4]);
        	$('#subvisit-consultant_doctor').html(obj[5]);
        	$('#subvisit-net_amt').val(obj[6]);
        	$('#cancellogtable-cancel_amt').val(obj[7]);
        	$('#cancellogtable-amt_words').val(obj[8]);
        	$('#cancellogtable-paid').val(obj[7]);
        	$('#cancellogtable-reason_cancelled').focus();
        	
        	$('#subvisit-sub_visit').attr('readonly','readonly');
 ...
 function Patient_List_Show(e)
 {
 	if(e.value != '')
 	{
 		$.ajax({
        type: "POST",
        url: "<?php echo Yii::$app->homeUrl . "?r=cancel-log-table/cancelkey&id=";?>"+e.value,
        success: function (result) 
        { 
 ...
 {		
 	if(event.keyCode == 13 && e.value != '')
	{
 		if(e.value != '')
 		{
 			var subvisit1=e.value;
 			
 			$.ajax({
	        type: "POST",
	        url: "<?php echo Yii::$app->homeUrl . "?r=cancel-log-table/cancelsubvisit&subvisit1=";?>"+subvisit1,
	        success: function (result) 
	        { 
	        	var obj = JSON.parse(result);
...
function SaveRegisterForm() 
{
	...
	if(valid == true)
	{
		...
		$.ajax({
	        type: "POST",
	        url: "<?php echo Yii::$app->homeUrl . "?r=cancel-log-table/cancelopd";?>",
	        success: function (result) 
	        { 
	        	if(result == 'S')
	        	{
	        		alert('Data Saved');
	        		$('.summited').attr('disabled','disabled');
