 ...
 <div class="col-md-6">
    <?= $form->field($model, 'no_of_unit')->textInput(['maxlength' => true,'onkeypress'=>'return isNumber(event)',]) ?>
     ...
     <?php
     	 ...
     	 echo $form->field($model, 'is_tablet', [
    			'template' => "<div class='checkbox checkbox-custom' style='margin-top:30px;'>{input}<label>Is Tablet</label></div>{error}",
])->checkbox([],false);
...
<hr>
       <div class="form-group col-md-12">
        ...
        <?= Html::submitButton($model->isNewRecord ? '<i class="fa fa-fw fa-save"></i>Save' : '<i class="fa fa-fw fa-edit "></i>Update', ['class' => $model->isNewRecord ? 'btn btn-success pull-right unit' : 'btn btn-primary pull-right updateunit']) ?>
