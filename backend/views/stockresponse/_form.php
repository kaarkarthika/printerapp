   ...
   <?= $form->field($model, 'request_code')->textinput(['readonly'=>true,'value'=>$model1->requestcode])->label(false); ?>
      ...
      <div class="col-md-2">
      	<label>Invoice Number</label>
      	          <?= $form->field($model, 'vendor_inv_no')->textinput(['required'=>true])->label(false); ?>
                </div> 
    <div class="col-md-2">
...
<div class="container">
			 ...
			 <table id="tablesample" class="table table-striped table-bordered" >
				...
				if($hsncode)
				{
					...
					//TaxGroupingLog
		$taxdata=TaxgroupingLog::find()->where(['taxgroupid'=>$hsncode])->andWhere(['is_active'=>1])->one();
           ...
           'class'=>'form-control'])->label(false);?>
		    ...
		    'placeholder'=>'priceperquantity',
		     ...
		     'required'=>true])->label(false);?>
                    ...
                    <div class="col-md-12">
					 <?php echo '
             	<button type="button" class="btn btn-sm btn-icon btn-success waves-effect waves-light addstock" >
									 <i class="fa fa-plus"></i>
									 </button> '; 
												  echo '
             	
									<button type="button" class="btn btn-sm btn-icon btn-danger waves-effect waves-light removebutton" id="removebutton'.$i.'">
		     ...
		     <div class="col-md-12">
             	...
             	<button type="button" class="btn btn-sm btn-icon btn-success waves-effect waves-light addstock" >
									 <i class="fa fa-plus"></i>
												  ...
												  echo '
             	<button type="button" class="btn btn-sm btn-icon btn-danger waves-effect waves-light removebutton">
									<i class="fa fa-trash"></i>
									 </button>'; 	 	
 ...
 <?php echo $form->field($model, 'gstvalue[]')->textInput([
   'id'=>'gstvalue'.$i, 'dataincrement'=>$i, 'placeholder'=>'GST Value',  'class'=>"form-control tabind",'tabindex'=>1017 ,'readonly'=>'true','required'=>true])->label(false);?>
	...
	<?php echo $form->field($model, 'igstvalue[]')->textInput([
        ...
        <?= Html::submitButton('Update', ['class' => 'btn btn-primary waves-effect waves-light update_req']) ?>
    </div>
    </div>
...
<script>
   $(document).on('keyup', '.receivedqty,.batchno,.receivedfreeqty,.unitid,.discountpercent,.gstpercent,.igstpercent,.mrpval', function ()
   {
          ...
          { var mrpperunit=(mrp/v11).toFixed(2);
          	mrpperunit=Math.round(mrpperunit);
          }
