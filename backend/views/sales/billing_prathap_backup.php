						    ...
						    <div class="col-sm-3">
						      <div class="form-group col-sm-7 " >
                                <div class="input-group add-on fwidth" >
                           		  <input class="form-control mrn inrefrsh number" placeholder="MRN Search" name="mr_number"  onkeyup="Patient_details(this.value,event)"  id="mrnumber" type="text" tabindex="8">
...
<table class="table table-bordered table-striped tbl-scrol" id="tbUser">
                        <thead>
                           <tr>
                              ...
                              <th rowspan="2" style="width:80px;" class="text-center">Stock / Drug</th>
                              <th rowspan="2" style="width:80px;" class="text-center">Batch</th>
                              <th rowspan="2" style="width:80px;" class="text-center">Exp Date</th>
                              <th rowspan="2" style="width:40px;" class="text-center">Qty</th>
                              <th rowspan="2" style="width:6.9%;" class="text-center">Unit<br>Form</th>
                              <th rowspan="2" width="7.6%" class="text-center">Price</th>
                             <!--  <th rowspan="2" width="5%" class="text-center">Discount Type</th> -->
                              <th colspan="2" width="15.6%"  class="text-center"  >Discount</th>
                             <!-- <th colspan="2" class="text-center">IGST</th> -->
                              <th colspan="2" width="13%"  class="text-center">CGST</th>
                              <th colspan="2" width="14%"  class="text-center">SGST</th>
                              <th rowspan="2" style="width:62px;" class="text-center">Total</th>
			    ...
			    <div class="col-sm-3" style="background-color: #ebeff2;margin-top:5px;position:relative;left:15px;" >
				   <div class=" ">
				       <table class="table"  >
						...
						<tr>
							...
							<td> 
								<div class="input-group">
		                          ...
		                          <input type="text" class="form-control total_disc_original ansrefrsh number" readonly='readonly' disabled name='total_disc_original'   id="total_discountamount">
						...
						<tr>
							<th>Round Off</th>
							<td><input type=" text" class="form-control text-right" id="round_off_value" readonly placeholder=" "></td>
						</tr>
						<tr>
							<th>Cash</th>
							<td><input type=" text" class="form-control text-right" id="cash_value" readonly placeholder=" "></td>
						</tr>
					 ...
					 <div class="panel">
                       <div class="panel panel-border">
                        <div class="panel-body" style="padding: 6px 7px 8px 35px!important;">					   
						   <button type="button" value='save_bill' name='saved_bill' style="z-index: 9999;" class="btn btn-success btn-sm fwidth ss_v save_billing" onclick='SaveBilling();' data-toggle="tooltip" title="Ctrl+s"><i class="fa fa-save"></i> Save</button>
						 <button type="button" class="btn  btn-sm btn-warning remove_all">Clear</button>
						
						 <a href="<?php  echo Yii::$app->request->BaseUrl;?>/index.php?r=sales/index" class="btn text-right btn-sm btn-default btn-bk" title="Back To Grid">Grid </a>
...
<script>
	$(document).ready(function(){
		$("#tbUser").on('click', '.delrow', function () 
		{
			...
			var total_value_amount=parseFloat($('#total_amount'+unique_id).val());
       ...
       var net_amount_in_words=convertNumberToWords(Math.round(delete_total_net_amount));
            $('#amt_in_words').val(net_amount_in_words);
      $('#cash_value').val(Math.round(delete_total_net_amount));
            
            //Round Off Value
            var round_off_value=Math.round(delete_total_net_amount);
            if(round_off_value > delete_total_net_amount)
            {
              var round_off_cal=round_off_value - delete_total_net_amount;
              $('#round_off_value').val(round_off_cal.toFixed(2));
            }
            else if(round_off_value < delete_total_net_amount)
            {
              var round_off_cal=delete_total_net_amount - round_off_value;
              $('#round_off_value').val(round_off_cal.toFixed(2));
            }
...
/*    $("body").on('keyup', '#over_all_discount_percent', function (e)
				...
				for (var i=0; i < arr.length; i++)
				{
					 ...
					 {	
						if(product_type != '')
						{
							...
							var fetchdata=$("#fetch_update_data tr").length;
              if(fetchdata!=1){
                 ...
                 $('#round_off_value').val('')
							...
							var markup = "<tr  class='save_data_table' data-id="+tbl_value[i]+" id='table_del"+tbl_value[i]+"'><td class='hide'>"+slno+"</td><td style='width:8.6%';><div class='trunctext '  style=' '  >"+product_name+"</div></td><td  style='width:8.7%';>"+batchnumber+"</td><td  style='width:8.6%';>"+expire_date_id+"</td><td style='width:4.4%'; class='quantity_add' id='quantity_add"+tbl_value[i]+"'>"+required_id+"</td><td  style='width:6.7%'; id='unit_value_medicine"+tbl_value[i]+"'>"+unit_value+"</td><td style='width:8%';><input type='hidden' name='medicine_type_ins[]' id='medicine_type_ins"+tbl_value[i]+"' value='"+unit_value+"'><input type='hidden' name='tablet_tot_unit_ins[]' id='tablet_tot_unit"+tbl_value[i]+"' value='"+total_unit+"'><input type='hidden' name='tablet_type[]' id='tablet_type"+tbl_value[i]+"' value='"+tablet_type+"'><input type='hidden' name='mrp_rate_per_unit[]' value='"+mrp_id+"'><input type='hidden' name='stock_id[]' value='"+stock_id+"'><input type='hidden' name='unit_id[]' value='"+unit_id+"'><input type='hidden' name='composition_id[]' value='"+composition_id+"'>"
							  							...
							  							+"<input type='hidden' name='batchnumber[]' value='"+batchnumber+"'><input type='hidden' name='product_name[]' value='"+product_name+"'><input type='hidden' name='quantity[]' value='"+required_id+"'><input type='hidden' name='primeid[]' value='"+tbl_value[i]+"'><input type='text' name='price[]' class='price_mrp text-right form-control w-50' data_price_mrp="+tbl_value[i]+" value="+price_per_quantity.toFixed(1)+" id="+'price'+tbl_value[i]+"></td>"
										...
										var product_type=$('#data_unit'+tbl_value[i]).val();
									  	...
									  	if(arr[i] != '' && tbl_value[i] != '')
									  	{
									  		if(product_type != '')
											{
												...
												var data_no_of_unit_value=$('#data_no_of_unit'+tbl_value[i]).val();	
												//alert(data_no_of_unit_value);
								var fetchdata2=$("#fetch_update_data tr").length;
              if(fetchdata2!=1){
                 ...
                 $('#round_off_value').val('')
												...
												var markup = "<tr  class='save_data_table' data-id="+tbl_value[i]+" id='table_del"+tbl_value[i]+"'><td class='hide'>"+slno+"</td><td style='width:8.6%';><div class='trunctext  '>"+product_name+"</div></td><td style='width:8.7%';>"+batchnumber+"</td><td style='width:8.6%';>"+expire_date_id+"</td><td style='width:4.4%'; class='quantity_add' id='quantity_add"+tbl_value[i]+"'>"+required_id+"</td><td style='width:6.7%'; id='unit_value_medicine"+tbl_value[i]+"'>"+unit_value+"</td><td style='width:8%';><input type='hidden' name='medicine_type_ins[]' id='medicine_type_ins"+tbl_value[i]+"' value='"+unit_value+"'><input type='hidden' name='tablet_tot_unit_ins[]' id='tablet_tot_unit"+tbl_value[i]+"' value='"+total_unit+"'><input type='hidden' name='tablet_type[]' id='tablet_type"+tbl_value[i]+"' value='"+tablet_type+"'><input type='hidden' name='mrp_rate_per_unit[]' value='"+mrp_id+"'><input type='hidden' name='stock_id[]' value='"+stock_id+"'><input type='hidden' name='unit_id[]' value='"+unit_id+"'><input type='hidden' name='composition_id[]' value='"+composition_id+"'>"
							  							+"<input type='hidden' name='stockcode_id[]' value='"+stockcode_id+"'><input type='hidden' name='brandcode_id[]' value='"+brandcode_id+"'><input type='hidden' name='product_name_id[]' value='"+product_id+"'><input type='hidden' name='expire_date_id[]' value='"+expire_date_id+"'>"
							  							+"<input type='hidden' name='batchnumber[]' value='"+batchnumber+"'><input type='hidden' name='product_name[]' value='"+product_name+"'><input type='hidden' name='quantity[]' value='"+required_id+"'><input type='hidden' name='primeid[]' value='"+tbl_value[i]+"'><input type='text' name='price[]' class='price_mrp text-right form-control w-50' data_price_mrp="+tbl_value[i]+" value="+price_per_quantity.toFixed(1)+" id="+'price'+tbl_value[i]+"></td>"
						  								//+"<td><ul class='donate-now'><input type='hidden' name='discount_method[]' id='disc_method"+tbl_value[i]+"' ><li><input type='radio'  name='desc"+tbl_value[i]+"' data_flat='"+tbl_value[i]+"' id='flat_discount"+tbl_value[i]+"'  disabled='disabled' class='deselect flat testrad'  onchange='descChanged("+tbl_value[i]+")'><label for='flat_discount"+tbl_value[i]+"' class='w-50 text-center testrad'>F</label></li><li>"
						  								//+"<input type='radio' id='percent"+tbl_value[i]+"' data-deselect="+tbl_value[i]+"  disabled='disabled' class='deselect percent testrad' name='desc"+tbl_value[i]+"'  onchange='descChanged("+tbl_value[i]+")' ><label for='percent"+tbl_value[i]+"' class='w-50 text-center testrad'>%</label></li></ul></td>"
                ...
                if(disc_percentage!=''){
                 OverallPercentageDiscount();
						...
						{	
							...
							var total_amount=parseFloat($('#price'+data_id).val());
							var total_quantity=parseInt($('#quantity_add'+data_id).html());
							var sgst=parseFloat($('#sgst_value'+data_id).val());
							var cgst=parseFloat($('#cgst_value'+data_id).val());
							var disc_amount=parseFloat($('#disc_amount'+data_id).val());
							var total_net_amount=parseFloat($('#total_amount'+data_id).val());
							if(isNaN(sgst))
   							{
   								sgst=0;
   							}
 ...
 function OverallPercentageDiscount()
 {
   ...
   // {
    	if($('#fetch_update_data tr').length > 0)
	    {
	    	var disc_percentage=$('#total_discountvaluetype').val();
         alert(disc_percentage);
          ...
          alert(cal_individual);
		 	 		...
		 	 		$('#enabledisc'+data_id).val(disc_percentage);
		 	 		$('#disc_amount'+data_id).val(cal_individual);
...
$("#mrnumber").typeahead({
  ...
  source: function(query,result) {
	  $.ajax(
	  {
		url:'<?php echo Yii::$app->homeUrl . "?r=sales/ajaxfetch";?>',
		method:'POST',
		data:{query:query},
		dataType:'json',
  ...
  {  
  		...
  		if(hidden_mr_number !== result)
  		{
	    	$('#load1').show();
			$.ajax({
		  			url:'<?php echo Yii::$app->homeUrl . "?r=sales/fetchmrnumber&mrnumber=";?>'+result,
		  			method:'POST',
		  			dataType:'json',
		  			success:function(data)
		  			{   
	            		...
	            		if(data[7] == 'N')
	            		{
	            			$('#pat_name').val(data[0]);
			           		$('#pat_mob').val(data[1]);
			           		$('#pat_doctor').val(data[2]);
							
			           		if(data[3] != '01-01-1970')
			           		{
			           			$('#pat_dob').val(data[3]);
			           		}
			           		document.getElementById('pat_insurance').innerHTML=data[4];
			           		$('#gender').html(data[5]);
			           		$('#address_patient').val(data[6]);
