				...
				foreach ($lab_payment as $key => $value)  
				{
					 ...
					 if($split_group[0]=="MasterGroup"){
						...
						if(!empty($mastergroupname)){
						 ...
						 $testgroup_list=LabAddgroup::find()->where(['mastergroupid'=>$value['lab_common_id']])->asArray()->all();
						 
						 foreach ($testgroup_list as $grpkey => $testgrp) {
							$testgroup_name=Testgroup::find()->where(['autoid'=>$testgrp['testgroupid']])->asArray()->one();
							 $lab_grouptest=LabTestgroup::find()->where(['testgroupid'=>$testgrp['testgroupid']])->asArray()->all();
