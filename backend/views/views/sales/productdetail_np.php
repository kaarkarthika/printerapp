...
$stockreceivedata=Stockresponse::find()->where(['stockresponseid'=>$stockresponseid])->all();
 ...
 $saledata=Sales::find()->where(['paid_status'=>'UnPaid'])->andwhere(['branch_id'=>$companybranchid])->all();
			...
			foreach ($stockreceivedata as $k) {
				  ...
				  $pid=$k->stockbrandcode->productid;
				  $hsncode=$k->stockbrandcode->product->hsn_code;
				
				
				$composition = Composition::find() -> where(['composition_id' => $k->stockbrandcode->compositionid]) -> one();
				
				$taxgroupdata=Taxgrouping::find()->where(['hsncode'=>$hsncode])->one();
				$tax=$taxgroupdata->tax;
			
				$unitlist=ArrayHelper::map(Unit::find()->where(['unitname'=>$k->stockbrandcode->product->product_typeid])->asArray()->all(), 'unitid', 'unitvalue');
				
				$custommrp=number_format(($k->mrpperunit)/(1+(0.01*$tax)),2);
 ...
 <!-- <td><?php echo $autonumber;?></td>-->
  <td><?php echo $k->stockbrandcode->product->productname; echo Html::input('hidden', 'productid[]', $pid);?></td>
  <td><?php echo $composition -> composition_name;echo Html::input('hidden', 'compositionid[]', $composition -> composition_id); ?></td>
  <td><?php echo $k->stockbrandcode->brandcode; echo "/"; echo $k->stockbrandcode->stockcode;
  
  
  echo Html::input('hidden', 'brandcode[]',$k->stockbrandcode->brandcode); ?>
<?php  echo Html::input('hidden', 'stock_code[]', $k->stockbrandcode->stockcode); ?></td>
<td><?php echo $k->batchnumber; echo "/";echo $availableqty;


echo Html::input('hidden', 'batchnumber[]', $k->batchnumber);echo Html::input('hidden', 'expiredate[]', $k->expiredate); ?></td>

<td style="width:150px;">
 <input type="text" name="productqty[]" value="0" class="form-control productqty_np" dataincrement="<?php echo $i;?>" id="quantity_np<?php echo $i;?>" required="true" placeholder="Quantity" />
 </td>
 <td>
 	 <?php   
...
<input type="hidden"  name="gst[]"  required="true"  value="0" dataincrement="<?php echo $i;?>" class="form-control gstpercent_np" id="gst_np<?php echo $i;?>"   style="text-align:right;"/>
 <input type="hidden" readonly="true" name="gst_value[]" class="form-control gstvalue_np" value="0" dataincrement="<?php echo $i;?>"  required="true" id="gst_value_np<?php echo $i;?>"   style="text-align:right;"/>
<?php } ?>
  <td>
...
$(function() {
          ...
          if(totalunit>totalstock)
          {
				...
				$("#totaldiscountnp").text("Rs." + totaldiscount.toFixed(2));
				var totalgst = 0;
