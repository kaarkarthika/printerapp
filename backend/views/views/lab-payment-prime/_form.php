                              ...
                              <div class="row">
							   ...
							   <div class="form-group col-sm-2 ">									<div class="input-group  ">
									...
									<ul class="donate-now per_flat_val">
										<li>
											<input type="radio" name="overall_discount_type" style="margin-left: 5px;" id="overall_discount_type_radio" value="F" class="numberfloat btn btn-success overall_disount">
											<label for="flat_discount" style="" class=" text-center testrad">F</label>
										</li>
										<li>
											<input type="radio" id="overall_percent_type" class="numberfloat btn btn-success overall_disount" value="P" name="overall_discount_type">
 ...
 $(document).ready(function() 
  {
    ...
    $("body").on('click', '.save_billing', function ()
    {
    	$.ajax({
	            type: "POST",
	            url: "<?php echo Yii::$app->homeUrl . "?r=lab-payment-prime/savedata";?>",
	            data: $("#w0").serialize(),
	            success: function (result) 
	            { 
   ...
   $("body").on('change', '.overall_disount', function ()
   {
	...
	$("body").on('keyup', '#total_discountamount', function (e)
    {
    	if($('#overall_discount_type_radio').is(":checked"))
        {
		        ...
		        {
		        	var net_amount=parseFloat($('#total_net_amount').val());
		        	
		        	if(!isNaN(net_amount))
		        	{
		        		var calculation=parseFloat((discount_value_flat*100)/net_amount);
		        		$('#total_discountvaluetype').val(calculation.toFixed(2));
		        		var net_amt=net_amount-discount_value_flat;
		        		$('#total_net_amount').val(net_amt.toFixed(2));
		        	}
		        	else if(isNaN(net_amount))
		        	{
		        		$('#total_discountamount').val('');
		        		$('#total_discountvaluetype').val('');
    ...
    $("body").on('keyup', '#total_discountvaluetype', function (e)
    {
    	if($('#overall_percent_type').is(":checked"))
        {
		    	...
		    	if(keycode == '13' && discount_value_flat != '')
		        {
		        	...
		        	if(!isNaN(net_amount))
		        	{
		        		var calcu=(net_amount*discount_value_flat)/100;
		        		var original_amount=net_amount-calcu;
		        		
		        		$('#total_discountamount').val(calcu.toFixed(2));
		        		$('#total_net_amount').val(original_amount.toFixed(2));
