...
$this -> title = "Approve Transfer Stock After Returned";
?>
<style>
	#load {
		display: none;
...
<div class="panel-body">
	...
	<table class="table table-striped">
		<thead><tr>
			<th>Request Code</th><th>Return Apporve Date</th><th>From Branch</th><th>To Branch</th>
		</tr></thead>
	...
	<div  style="white-space: nowrap; overflow-x: visible;overflow-y: hidden;width: 1040px;">   
	 	<table  class="table table-striped table-bordered">
             ...
             <tbody>
             	<?php
             	$i=1;
				$data=	Transferstockreturn::find()->where(['transferstock_requestcode'=>$requestcode])->all();
				...
				$modelxz=Transferstockreturn::find()->where(['transferstockid'=>$value->transferstockid])->one();
				$modelstock = Stockmaster::find()->where(['productid'=>$modelz->productid])->andwhere(['branch_id'=>$model1->tobranch])
				->one();
				
	echo $form->field($model, 'transferstockreturnid[]')->hiddenInput(['id'=>'transferstockreturnid'.$i,'name'=>'transferstockreturnid'.$i,'value'=>$value->transferstockreturnid])->label(false).'</td>';			
                         $receivedata= Transferstockreturnapprove::find()->where(['transferstockreturnid'=>$value->transferstockreturnid])->one();
						 if($receivedata)
						 {
				...
				<tr>
					...
					<td width="50px">'.$value->returnquantity."-".$unitdata->unitvalue.'</td>';
