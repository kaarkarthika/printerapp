...
if(!$approvedmodel)
{
									...
									if($qty==0)
									{
										...
										echo '<button type="button" id="btnsubmit'.$i.'" dataincrement="'.$i.'" class="btn btn-sm btn-icon btn-default waves-effect waves-light choose_apstock"data-tsid='.$value->transferstockid.'
...
<td style="text-align:right;"><span id="total">Rs.0</span><input type="hidden" id="totalprice" name="totalprice" /></td></tr></tbody>
      ...
      <table id="approvedgrid"  class="table table-striped table-hover table-bordered">
        ...
        <th>Batch Number</th><th>Approved Quantity</th><th>Unit</th><th>Total Units</th> <th>Price/Qty</th><th>Total Price</th></tr>
             </thead>  <?php $i=0;?>
  ...
  <div class="form-group pull-right" id="approvedbtn" style="display:none;">
        <?php echo Html::Button('Update', ['class' => 'btn btn-default waves-effect waves-light update_req']) ?>
    </div>
      </div>
        <?php ActiveForm::end(); ?>
		</div>
