...
if($estimate_main_tbl['convert_status'] == 'DC') 
{

 $delivery_fetch=Delivery::find()->where(['id'=>$estimate_main_tbl['convert_delivery_id']])->asArray()->one();

  $delivery_ref_fetch=DeliveryRef::find()->where(['delivery_id'=>$estimate_main_tbl['convert_delivery_id']])->asArray()->all();
}
...
<div class="col-md-4">
    ...
    <?= $form->field($model, 'company_name')->textInput(['readonly'=> true,'maxlength' => true,'class'=>'form-control input-sm typehead','onkeyup'=>'EmptyEsc(this.value,event);','value'=>$estimate_main_tbl['company_name']])->label('COMPANY NAME') ?>
  
     <input type="hidden" id="custId" name="custId" value="<?php echo $estimate_main_tbl['customer_id']; ?>">
  
      <?= $form->field($model, 'cust_name')->hiddenInput(['readonly' => true])->label(false) ?>

      <input type="hidden" id="convertdc" name="CONVERTDC" value="<?php echo $estimate_main_tbl['id']; ?>">
</div>

<div class="col-md-2">
    <?= $form->field($model, 'gstin_no')->textInput(['readonly' => true,'value'=>$customer_master_fetch['gst']])->label('GSTIN') ?>
</div>

<div class="col-md-2">
          <?= $form->field($model, 'gst_type')->dropDownList(['GST'=>'GST','IGST'=>'IGST'],['options' => [$estimate_main_tbl['gst_type'] => ['Selected'=>'selected']],'class' => 'form-control input-sm gst_percent','id'=>'gst_percent','prompt'=>"SELECT",'style'=>'pointer-events:none;'])->label('GST Type') ?>
      </div>
...
<div class="col-md-2">
 ...
 <div class="col-md-2">
    ...
    <br/>
      ...
      <b style="color: blue;"><?php echo date('d-m-Y',strtotime($model->bill_no));?></b>
...
<div class="col-md-12">
    <?= $form->field($model, 'address')->textArea(['readonly' => true,'value'=>$customer_master_fetch['address']])->label('Billing Address') ?>
</div>
...
<div class="col-md-7">
    <?= $form->field($model, 'delivery_address')->textArea(['readonly' => true,'value'=> $estimate_main_tbl['convert_status'] == '' ? '' :  $delivery_fetch['delivery_address']])->label('Delivery Address') ?>
</div>
...
<div class="ot-job-form">
    <div class="col-md-12">
            ...
            <?php 
            $k=1;
            if($estimate_main_tbl['convert_status'] == '')
            {
                if(!empty($estimate))
                {
    ...
    <td><input type="text" class="form-control input-sm description_table disabled_field" required id="description_table<?php echo $k; ?>" value="<?php echo $value['particular_field']; ?>" name="DeliveryRef[description_table][]"/></td>


    <td><input type="text" class="form-control input-sm qty_table disabled_field" id="qty_table<?php echo $k; ?>" name="DeliveryRef[qty_table][]"/ required></td>


    <td><input type="text" class="form-control input-sm hsn_table disabled_field" id="hsn_table<?php echo $k; ?>" name="DeliveryRef[hsn_table][]"/></td>


    <td>
      <?= $form->field($delivery_ref, 'gst_rate')->dropDownList($tax_master,['options' => [$estimate_main_tbl['gst'] => ['Selected'=>'selected']],'prompt'=>'-SELECT-','class' => 'form-control input-sm gst_rate','id'=>'gstrate_table'.$k,'name'=>'DeliveryRef[gst_rate_table][]'])->label(false) ?>
      <!--input type="hidden" class="form-control input-sm hsn_table disabled_field" id="gstrate_table"  name="DeliveryRef[gst_rate_table][]"/-->
    </td>

      <td>

         <?= $form->field($delivery_ref, 'amount')->textInput(['class' => 'decimal_validated form-control input-sm no-padding amount','id'=>'amount_table'.$k,'name'=>'DeliveryRef[amount_table][]','value'=>$value['amount']])->label(false) ?>

        <!--input type="hidden" class="form-control input-sm amount_table disabled_field" readonly id="amount_table<?php //echo $k; ?>" value=" <?php //echo $value['amount']; ?>" name="DeliveryRef[amount_table][]"/-->
    ...
    <td style="text-align:center;"><button type="button" class="btn btn-danger btn-xs disabled_field" style="text-align:center;" onclick="Remove('<?php echo $k; ?>');">Remove</button></td></tr>
          ...
          else if($estimate_main_tbl['convert_status'] == 'DC') 
          {

           

            if(!empty($delivery_ref_fetch))
            {

                foreach ($delivery_ref_fetch as $key => $value) {
    ...
    <tr data-id='<?php echo $k; ?>' id="row_id<?php echo $k; ?>" class="row_id">
    <td data-id='<?php echo $k; ?>' id="sl_id<?php echo $k; ?>" class="sl_id"><?php echo $k; ?></td>
    <td><input type="text" class="form-control input-sm description_table disabled_field" required id="description_table<?php echo $k; ?>" value="<?php echo $value['description']; ?>" name="DeliveryRef[description_table][]"/></td>


    <td><input type="text" class="form-control input-sm qty_table disabled_field" id="qty_table<?php echo $k; ?>" name="DeliveryRef[qty_table][]" value="<?php echo $value['qty']; ?>" required></td>


    <td><input type="text" class="form-control input-sm hsn_table disabled_field" id="hsn_table<?php echo $k; ?>" name="DeliveryRef[hsn_table][]" value="<?php echo $value['hsn']; ?>" /></td>


    <td>
      <?= $form->field($delivery_ref, 'gst_rate')->dropDownList($tax_master,['options' => [$value['gst_rate'] => ['Selected'=>'selected']],'prompt'=>'-SELECT-','class' => 'form-control input-sm gst_rate','id'=>'gstrate_table'.$k,'name'=>'DeliveryRef[gst_rate_table][]'])->label(false) ?>
      <!--input type="hidden" class="form-control input-sm hsn_table disabled_field" id="gstrate_table"  name="DeliveryRef[gst_rate_table][]"/-->
    </td>
...
<div class="col-md-12">
     ...
     <div class="col-md-3">
      ...
      <?php if($estimate_main_tbl['convert_status'] == 'DC') { ?>
    <?= $form->field($model, 'transport')->dropDownList($transport_array,['options' => $estimate_main_tbl['convert_status'] == 'DC' ? [$delivery_fetch['transport'] => ['Selected'=>'selected']] : '','prompt'=>'--SELECT--','class' => 'form-control input-sm'])->label('Purpose Of Transport') ?>


      <?php }else if($estimate_main_tbl['convert_status'] == '') {   ?>
  ...
  <div class="col-md-5">
      <?= $form->field($model, 'vehicle_num')->textInput(['class' => 'form-control input-sm','value' => $estimate_main_tbl['convert_status'] == 'DC' ? $delivery_fetch['vehicle_num'] : ''])->label('To Whom delivered for Transport and Vehicle No if any') ?>
</div>

 <div class="col-md-4">
      <?= $form->field($model, 'remarks')->textArea(['class' => 'form-control input-sm','value' => $estimate_main_tbl['convert_status'] == 'DC' ? $delivery_fetch['remarks'] : ''])->label('Remarks') ?>
</div>
...
<div class="col-md-12">
 <div class="pull-right">
        ...
        <label class="control-label" style="visibility: hidden;" for="delivery-remarks">Remarks</label>
         <?= Html::submitButton($estimate_main_tbl['convert_status'] == '' ? '<i class="fa fa-fw fa-save"></i>Save & Print' : '<i class="fa fa-fw fa-edit "></i>Update & Print', ['value'=>'yes','id'=>$estimate_main_tbl['convert_status'] == '' ? 'savedprint' : 'updatedprint' ,'class' => $estimate_main_tbl['convert_status'] == '' ? 'btn btn-success pull-right tax' : 'btn btn-success updatetax']) ?>
     </div>
 <div class="pull-right">
 
        <label class="control-label" style="visibility: hidden;" for="delivery-remarks">Remarks</label>
        <?= Html::submitButton($estimate_main_tbl['convert_status'] == '' ? '<i class="fa fa-fw fa-save"></i>Save' : '<i class="fa fa-fw fa-edit "></i>Update', ['value'=>'no','id'=>$estimate_main_tbl['convert_status'] == '' ? 'saved' : 'updated' ,'class' => $estimate_main_tbl['convert_status'] == '' ? 'btn btn-primary pull-right tax' : 'btn btn-primary updatetax']) ?>
  </div>
   <div class="pull-right">
...
<?php 
    ...
    <?php } else if($estimate_main_tbl['convert_status'] == 'DC') {
    ?>

       var comments_save='Are You Sure to Update ?'; 
       var comments_print='Are You Sure to Update & Print ?'; 

       var print_url='<?php echo Yii::$app->homeUrl ?>?r=delivery/updateprint&id='+'<?php echo $delivery_fetch['id']; ?>';
       var save_url='<?php echo Yii::$app->homeUrl ?>?r=delivery/update&id='+'<?php echo $delivery_fetch['id']; ?>';
...
<?php if(!empty($delivery_address_master_json)) { ?>

var obj=$.parseJSON('<?php echo $delivery_address_master_json; ?>');
...
<?php } else if(empty($delivery_address_master_json)){ ?>
...
function Add_To_Grid()
{
  ...
  <?php }?>
    ...
    '<td><input type="text" class="form-control input-sm qty_table disabled_field" id="qty_table'+current+'" name="DeliveryRef[qty_table][]" value="'+qty+'" required></td>'+
    '<td><input type="text" class="form-control input-sm hsn_table disabled_field" id="hsn_table'+current+'" name="DeliveryRef[hsn_table][]"  value="'+hsn+'"></td>'+
    '<td><select id="gstrate_table'+current+'" class="form-control input-sm gst_rate" name="DeliveryRef[gst_rate_table][]">'+
