	   ...
	   <div class="row">
         <div class="panel panel-border panel-custom">
            ...
            <div class="panel-body">
			   <div class="row">
				<div class="col-sm-12">
		          <div class=" "  style="position: relative;top: 2px;">
				  ...
				  <div class="col-sm-9">
				   ...
				   <div class=" ">
				       ...
				       <table class="table">
						...
						<tr>
							<th>GST</th>
							<td><input type="text"   class="form-control total_vat ansrefrsh text-right" name='total_gst' readonly id="total_gst_amount">
										<input type="hidden"  class="form-control total_vat ansrefrsh text-right" name='total_gst_hidden'  id="total_gst_amount_hidden">
									</tr>
						<tr>
							<th>Bill Total</th>
						...
						<tr>
							<th>NET Amount</th>
							<td><input type="text" class="form-control f-14 total-netamt ansrefrsh bg-primary text-right" name='total_net_amount'   readonly id="total_net_amount">
										<input type="hidden" class="form-control f-14 total-netamt ansrefrsh bg-primary text-right" name='total_net_amount_hidden'  id="total_net_amount_hidden">
									</tr>
						<tr>
							<th>Sub Total</th>
							<td><input type="text" class="form-control total_sub_total text-right ansrefrsh" name='total_sub_total' readonly id="total_sub_total">
					 ...
					 <div class="panel">
                       <div class="panel panel-border">
						...
						<button type='reset' style="z-index: 9999;" class="inp clear_field btn btn-default btn-sm fwidth ss_v remove_all" data-toggle="tooltip" title="Ctrl+c">Close</button>						 
			           </div>
			          </div>
			          </div>
				   </div>
...
<!--
	 ...
	 <strong><span class="pull-right" id="date_time"></span></strong>
           <div class="panel panel-border panel-custom home-body"> <!-- ss  
            ...
            <div class="panel-body">
             <div class="row">
                <a href="<?php  echo Yii::$app->request->BaseUrl;?>/index.php?r=in-sales/index" class="btn btn-bk btn-default b-width " style="float:right;" Title="Go To Grid">Go to Grid </a> 
              </div>   
               <div class="row">
     		<div class="col-sm-12">
...
<?php  $url1 = Yii::$app->homeUrl .'?r=in-sales/opreturns';  ?>
...
$(".ipnumber").typeahead({
  ...
  source: function(query,result) {
        $.ajax({
          url:'<?php echo Yii::$app->homeUrl . "?r=in-sales/ajaxbillnumber";?>',
          method:'POST',
          data:{query:query},
          dataType:'json',
  ...
  {  $('#load1').show();
    $.ajax({
          url:'<?php echo Yii::$app->homeUrl . "?r=in-sales/opdetails&id=";?>'+result,
          method:'POST',
          dataType:'json',
          success:function(data)
          {   $('#load1').hide();
   ...
   $("body").on('click', '.delrow', function () 
  {
    ...
    $('#total_sub_total').val(parseFloat($('#total_sub_total').val()).toFixed(2)-parseFloat(total_sub_total).toFixed(2));
    ...
    $('#no_of_items').val($("#fetch_update_data tr").length-1); 
...
+"<td><div class='trunctext'>"+product_name+"</div></td><td style='width:4%;'>"+batchnumber+"</td><td style='width:8%;'>"+expire_date_id+"</td>"
+"<td style='width:5%;' class='quantity_add' id='quantity_add"+tablet_id+"'>"+required_id+"</td><td style='width:7%;' id='unit_value_medicine"+tablet_id+"'>"+unit_value+"</td>"
...
+"<td class=''><input type='hidden' class='form-control' data_gst_percent='"+tablet_id+"' name='gst_percent[]' id='gst_percent"+tablet_id+"' value='"+gst_sale_percent+"' readonly><input type='text' value='"+cgst_percent+"' class='w-53 disctxt cgst_percent text-right' name='cgst_percent[]' data_cgst_percent='"+tablet_id+"' id='cgst_percent"+tablet_id+"' readonly></td>"
+"<td><input type='text' value='"+cgst_value.toFixed(2)+"' class='w-53 cgst_value text-right disctxt' name='cgst_value[]' data_cgst_value='"+tablet_id+"' id='cgst_value"+tablet_id+"' readonly></td>"
+"<td class='w-xss'><input type='text'  class='w-53 sgst_percent disctxt' value='"+sgst_percent+"' name='sgst_percent[]' data_sgst_percent='"+tablet_id+"' id='sgst_percent"+tablet_id+"' readonly></td>"
+"<td><input type='text' value='"+sgst_value.toFixed(2)+"' class='w-53 sgst_value text-right disctxt' name='sgst_value[]' data_sgst_value='"+tablet_id+"' id='sgst_value"+tablet_id+"' readonly></td>"
+"<td><input type='text'  class='w-53 disctxt total_amt_cal text-right' value='"+total_amount+"' name='total_amt_cal[]' data_total='"+tablet_id+"' id='total_amount"+tablet_id+"' readonly>"
