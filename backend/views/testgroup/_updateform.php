...
//echo"<pre>"; print_r($resultgroup[0]['autoid']); die;
?>
...
<div class="testgroup-form">
 ...
 <div class="row"> 	
	<div class="col-sm-2"> 
		<?php $auto_id=$resultgroup[0]['autoid'];  ?>
	<!-- <input type="hidden" name="autoid" class="autoid" value="<?php echo $autoid; ?>"> -->
	 <?= $form->field($model, 'testgroupname')->dropdownlist($testgrouplist,
    ['prompt'=>'Select Category','class'=>'selectpicker testgroupname', 'data-live-search'=>'true',
     'data-style'=>"btn-default btn-custom1",'required' => true,'id'=>'user_dropdown'],    
          ['options' =>
                   [$auto_id => ['Selected'=>'selected']]
          ]); ?>
          
     	</div>
   	<div class="col-sm-2">
    <?= $form->field($model, 'price')->textInput(['required' => true, 'value' => $resultgroup[0]['price']]) ?>
    	</div>
    	<div class="col-sm-2">
		 	<?= $form->field($model, 'hsncode')->textInput(['maxlength' => true,'required' => true,'value' => $resultgroup[0]['hsncode']]) ?>
