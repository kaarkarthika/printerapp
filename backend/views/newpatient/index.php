...
<div class="col-sm-12">
	...
	<?= Html::a('Sub-Visit', ['sub-visit-new1'], ['class' => 'btn btn-primary']) ?>
	</div>
 <div class="btn-group pull-right m-t-15">
			...
			['attribute' => 'patientname', 
			  ...
			  ['attribute' => 'pat_mobileno', 
            	...
            	'label' => 'UCIL ID',
             	'value'=> function($model)
				{
					if($model->subvisit->ucil_emp_id !=''){
					return $model->subvisit->ucil_emp_id;
					}else{
						return '--NIL--';
					}
				}
            	...
            	'label' => 'UCIL STATUS',
             	'value'=> function($model)
				{
					if($model->subvisit->ucil_letter_status =='YES'){
					return $model->subvisit->ucil_letter_status;
					}else if($model->subvisit->ucil_letter_status =='NO'){
						return $model->subvisit->ucil_letter_status;
					}
					else {
						return '-';
			...
			/*	 ['attribute' => 'doctor_name', 
             	...
             	'value'=> function($model)
				{
					if($model->mr_no!='')
					{
						...
						return $model->subvisit->consultant_name.'/'.date('d-m-Y h:i A',strtotime($model->subvisit->created_at));
...
*/
                               ...
                               'update' => function ($url, $model, $key) {
                              	//print_r($url);die;
									...
									'sub-visit' => function ($url, $model, $key) {
                                        ...
                                        return Html::a('<span class="fa fa-plus-square"></span>  Block', $url, $options);
									}
									else if($model -> temporary_blocked == 'N') {
										$options = array_merge([
                                            'class' => 'btn btn-primary btn-xs update gridbtncustom',
                                            'data-toggle'=>'tooltip',
                                            'title' => Yii::t('yii', 'SUB VISIT'),
                                ...
                                'delete' => function ($url, $model, $key) {
								  ...
								  'images' => function ($url, $model, $key) {
                                   ...
                                   // return Html::a('<span class="glyphicon glyphicon-eye-open"></span>', $url, $options);
                                   return Html::button('<i class="fa fa-file-image-o" aria-hidden="true"></i>', ['value' => $url, 'style'=>'margin-right:4px;','class' => 'btn btn-primary btn-xs view view gridbtncustom modalImage', 'data-toggle'=>'tooltip', 'title' =>'Image' ]);
								 },			 
    ...
    <?php 
    Modal::begin([
                    'header' => '<h4 id="operationalheader_large"><span> <i class="fa fa-fw fa-th-large"></i>Visiting details</span> </h4>',
    ...
    $(document).ready(function(){
     ...
     $('body').on("click",".modalView",function(){
	  ...
	  $('body').on("click",".modalImage",function(){
             var PageUrl = $(this).attr('value');
             $('#operationalheader').html('<span> <i class="fa fa-fw fa-th-large"></i>Patient Images</span>');
             $('#operationalmodal').modal('show').find('#modalContenttwo').load(PageUrl);
             return false;
