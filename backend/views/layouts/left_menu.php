...
if ($session['user_logintype']=='BA'  || $session['user_logintype']=='CA') {
	if($session['user_logintype']=='BA'){	
		...
		if(count($in_module)>0){
			/*if($_SESSION['menu_msater_session']=='MEMU_SESSION_MASTER'){
			$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andwhere(['is_master'=>1])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
			$ProjectModule=ArrayHelper::toArray($ProjectModuledb);	
			}else{
			$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andwhere(['is_master'=>NULL])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
			$ProjectModule=ArrayHelper::toArray($ProjectModuledb);
			}*/
			if($_SESSION['menu_rights_session']=='D')
			{
				if($_SESSION['menu_master'] == 'FRONT')
				{
					$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andWhere(['is_rights'=>'D'])->andWhere(['is_master'=>'1'])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
					$ProjectModule=ArrayHelper::toArray($ProjectModuledb);
				}
				else 
				{
					$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andWhere(['is_rights'=>'D'])->andWhere(['is_master'=>NULL])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
					$ProjectModule=ArrayHelper::toArray($ProjectModuledb);
				}
			}
			else if($_SESSION['menu_rights_session']=='P')
			{
				if($_SESSION['menu_master'] == 'FRONT')
				{
					$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andWhere(['is_rights'=>'P'])->andWhere(['is_master'=>'1'])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
					$ProjectModule=ArrayHelper::toArray($ProjectModuledb);
				}
				/*else if($_SESSION['menu_master'] == 'BACK')
				{
					$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andWhere(['is_rights'=>['P','M']])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
					$ProjectModule=ArrayHelper::toArray($ProjectModuledb);
				}*/
				else
				{
					$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andWhere(['is_rights'=>'P'])->andWhere(['is_master'=>NULL])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
					$ProjectModule=ArrayHelper::toArray($ProjectModuledb);
				}
				
			}
			else if($_SESSION['menu_rights_session']=='R')
			{
				if($_SESSION['menu_master'] == 'FRONT')
				{
					$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andWhere(['is_rights'=>'R'])->andWhere(['is_master'=>'1'])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
					$ProjectModule=ArrayHelper::toArray($ProjectModuledb);
				}
				else
				{
					
					$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andWhere(['is_rights'=>'R'])->andWhere(['is_master'=>NULL])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
					$ProjectModule=ArrayHelper::toArray($ProjectModuledb);
					
				}
				
			}
			else if($_SESSION['menu_rights_session']=='U')
			{
				if($_SESSION['menu_master'] == 'FRONT')
				{
					$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andWhere(['is_rights'=>'U'])->andWhere(['is_master'=>'1'])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
					$ProjectModule=ArrayHelper::toArray($ProjectModuledb);
				}
				else 
				{
					$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andWhere(['is_rights'=>'U'])->andWhere(['is_master'=>NULL])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
					$ProjectModule=ArrayHelper::toArray($ProjectModuledb);
				}
				
			}
			else if($_SESSION['menu_rights_session']=='L')
			{
				if($_SESSION['menu_master'] == 'FRONT')
				{
					$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andWhere(['is_rights'=>'L'])->andWhere(['is_master'=>'1'])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
					$ProjectModule=ArrayHelper::toArray($ProjectModuledb);
				}
				else 
				{
					$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andWhere(['is_rights'=>'L'])->andWhere(['is_master'=>NULL])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
					$ProjectModule=ArrayHelper::toArray($ProjectModuledb);
				}
			}
			else if($_SESSION['menu_rights_session']=='')
			{
				if($_SESSION['menu_master'] == 'FRONT')
				{
					$ProjectModuledb=AuthProjectModule::find()->where(['IN','p_autoid',$in_module])->andwhere(['is_active'=>1])->andWhere(['is_rights'=>'D'])->andWhere(['is_master'=>'1'])->orderBy(['sortOrder'=>'SORT_ASC'])->all();		
	...
	foreach($ProjectModule as $one_rt){
		...
		if($_SESSION['menu_master']=='FRONT'){
			...
			$menu_data_array_n[1] = array('one', 'Go to Dashboard', Yii::$app -> homeUrl."?r=sales/dashboard" , '<i class="fa fa-reply"></i>', '');
		}
		/*else if($_SESSION['menu_master']=='BACK'){
		$menu_data_array_n[0] = array('one', 'Dashboard', Yii::$app -> homeUrl, '<i class="fa fa-dashboard"></i>', 'index');
		}*/
		else {
			$menu_data_array_n[1] = array('one', 'Masters', Yii::$app -> homeUrl."?r=sales/master", '<i class="fa fa-share" aria-hidden="true"></i>', 'index');
		}
		//$menu_data_array_n[0] = array('one', 'Dashboard', Yii::$app -> homeUrl, '<i class="fa fa-dashboard"></i>', 'index');
	foreach($ProjectModule as $one_rt){
		if($one_rt['moduelRoot']==""){
...
$menu_data_array[3]['sub'][1]=array('View Cash Report',Yii::$app->homeUrl.'?r=invoice-excelupload','<i class="material-icons"></i>','invoice-excelupload','invoice-excelupload');	
	...
	//echo '<pre>';print_r($menu_data_array);die;
