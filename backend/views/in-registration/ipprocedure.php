...
input#intreatmentindividual-rate, input#intreatmentindividual-qty {
    text-align: right;
}
...
input#intreatmentoverall-discount_authority {
    text-align: left !important;
}
.finanical {
...
function PaidAmountCalculation(data,event) 
{
	...
	if(overall_net_amount >= paid_amount)
	{
		...
		$('#intreatmentoverall-overall_due_amount').val(amount);	
	}
...
function formatDate(date) 
{
 ...
 function SaveProcedures()
 {
	...
	if(valid === true)
	{
		...
		$.ajax({	
			     type: "POST",
 				 url: "<?php echo Yii::$app->homeUrl . "?r=in-registration/ipprocedure";?>",
 				 data: $("#w0").serialize(),
			     success: function (result) 
			     {
			     	if(result === 'S')
  ...
  $("body").on('click', '.delrow', function () 
	{
		...
		$('#intreatmentoverall-overalltotal').val(parseFloat($('#intreatmentoverall-overalltotal').val()).toFixed(2)-total_sub_total);
		$('#intreatmentoverall-overalldiscountpercent').val();
		$('#intreatmentoverall-overalldiscountamount').val();
