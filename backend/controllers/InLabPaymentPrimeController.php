...
class InLabPaymentPrimeController extends Controller
{
     ...
     public function actionAjaxipnumberselectblockipentries($id)
    {
        ...
        if(!empty($id))
        {
            ...
            if(!empty($Newpatient))
            {
            	$physicianmaster123=Physicianmaster::find()->where(['id'=>$sub_visit['consultant_doctor']])->asArray()->one();
				$insurance=Insurance::find()->where(['insurance_typeid'=>$sub_visit['insurance_type']])->asArray()->one();
				...
				$response[4] = $insurance['insurance_type'];
    ...
    public function actionBillreport($id)
    {
		...
		$contact_details=NewPatient::find()->where(['mr_no'=>$labprime_list['mr_number']])->asArray()->one();
        //echo"<pre>";	print_r($contact_details['pat_mobileno']);	 die;
        ...
        require ('../../vendor/tcpdf/tcpdf.php');
                ...
                <tr> <td style="width:15%;"> Requisition No </td><td style="width:35%;">: '.$labprime_list['mr_number'].'</td>
                    <td style="width:15%;"> Bill Number </td><td style="width:35%;">: '.$labprime_list['bill_number'].'</td> </tr>
                ...
                <tr><td style="width:15%;"> Contact Number </td><td style="width:35%;">: '.$contact_details['pat_mobileno'].' </td>
    ...
    public function actionAjaxipnumberselectblockipentriesdata($id)
    {
		...
		if(!empty($id))
		{
			...
			if(!empty($Newpatient))
			{
				...
				$physicianmaster_data=Physicianmaster::find()->where(['id'=>$Newpatient['consultant_dr']])->asArray()->one();
                $physicianmaster1_data=Physicianmaster::find()->where(['id'=>$Newpatient['co_consultant']])->asArray()->one();
				...
				$response[3] = $patienttype;
				$response[4] = $physicianmaster_data;
				$response[5] = $physicianmaster1_data;
			//	echo "<pre>"; print_r($response); die;	
				return json_encode($response);
			}
		}
