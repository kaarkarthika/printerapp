...
/*	 public function actionCreatedummy4() {
	...
	public function actionFetchmrnumber($mrnumber)
	{
		...
		$subvisit=SubVisit::find()->where(['sub_id'=>$mrnumber])->orderBy(['sub_id' => SORT_DESC])->one();
		...
		if(!empty($subvisit))
		{
			...
			$patientdata=Newpatient::find()->where(['patientid'=>$mrnumber])->orderBy(['patientid' => SORT_DESC])->one();
				...
				$fetch_element[0]=$patientdata->mr_no;
				$fetch_element[1]=$patient_name;
				$fetch_element[2]=$patient_mobile_number;
				$fetch_element[3]=$doctor_name_fetch;
				$fetch_element[4]=$dob;
				$fetch_element[5]=$insurances_name;
				$fetch_element[6]=$patientdata->pat_sex;
				$fetch_element[7]=$patientdata->pat_address;
				$fetch_element[8]=$patientdata->temporary_blocked;
	...
	public function actionSaveddata()
    {
		...
		//	print_r($_POST);die;
			...
			$paid_amt=$_POST['paid_value'];
			$due_amt=$_POST['due_value'];
			...
			{
				$subvisit=SubVisit::find()->where(['mr_number'=>$_POST['mr_number']])->orderBy(['sub_id' => SORT_DESC])->one();
				...
				if($_POST['get_temp_no'] == '')
				{
						...
						if($saledata ->save())
						{
							...
							$mooneylog= new SalesMoneyreceiptsLog();
							$mooneylog->mr_no=$_POST['mr_number'];
							$mooneylog->sales_money	=$saleid;
							$mooneylog->total_paid=$total_net_amount;	
							$mooneylog->bill_number	=$billformat;
							$mooneylog->prev_cashpaid=$saledata->paid_amt;
							$mooneylog->mode_of_payment="1";
							$mooneylog->bill_amount=$due_amt;
							$mooneylog->amount	=$saledata->paid_amt;
							$mooneylog->total_amount=$total_net_amount;
							$mooneylog->remark=$remarks;
							$mooneylog->created_at=date('Y-m-d H:i:s');
							$mooneylog->user_id=$session['user_id'];
							$mooneylog->ip_address=$_SERVER['REMOTE_ADDR'];
						
							if($mooneylog->save())
							 {
							 	
							 }else{
							 	print_r($mooneylog->getErrors()); die;
							 }
				...
				elseif ($_POST['get_temp_no'] != '') 
				{
					...
					if(!empty($saledetaildata))
					{
						...
						if(!empty($saletempinsert))
						{
							...
							$saletempinsert->billnumber='ECDR'.$saletempinsert->opsaleid;
							$saletempinsert->gender=$_POST['Gender'];
							$saletempinsert->address=$_POST['Address'];
			...
			else if($_POST['patient'] == 'outpatient')
			{
				if($_POST['get_temp_no'] == '')
				{
					/*$patientdata->patientname=$_POST['patient_name'];
					$patientdata->pat_mobileno=$_POST['mobile_number'];
				...
				/*	$patientdata->user_role=$session['user_id'];
					...
					//{
						...
						$Out_Primeid=$out_patient_array['patientid'];*/
									...
									$saledata->sur_name=$_POST['sur_name'];
									$saledata->name=$_POST['patient_name'];
									$saledata->dob=date('Y-m-d',strtotime($_POST['sales_dob']));
									$saledata->age=$_POST['sales_age'];
									$saledata->gender=$_POST['patient_gender'];
									$saledata->phonenumber=$_POST['mobile_number'];
									$saledata->address=$_POST['out_patient_address'];
									$saledata->patienttype=$_POST['sales_patient_type'];
	...
	public function actionReturntablet($id)
    {
			...
			if(!empty($Sale_detail))
			{
				...
				foreach ($Sale_detail as $key => $value)
				{
					...
					$result_string1.='<td style="width:4.4%;" class="quantity_add" id="quantity_add'.$value['stockresponseid'].'" style="color:red;font-weight:bold;">'.$value['productqty'].'</td>';
					...
					$result_string1.='<td><div class="input-group"> <input type="text" name="discount_value[]" data_disc_value='.$value['stockresponseid'].'  disabled="disabled" id="enabledisc'.$value['stockresponseid'].'" class="enabledisc disctxt w-55" readonly></div>
					...
					$result_string1.='<td class=""><input type="text"  class="form-control cgst_percent text-right" name="cgst_percent[]" data_cgst_percent='.$value['stockresponseid'].' id="cgst_percent'.$value['stockresponseid'].'" value="'.$value['cgst_percent'].'" readonly></td>';
					...
					$result_string1.='<td class=""><input type="text"  class="form-control sgst_percent" name="sgst_percent[]" data_sgst_percent='.$value['stockresponseid'].' id="sgst_percent'.$value['stockresponseid'].'"  value="'.$value['sgst_percent'].'"  readonly></td>';
			...
			if(!empty($Sale_detail))
			{
				...
				foreach ($Sale_detail as $key => $value)
				{
					...
					$result_string1.='<td><ul class="donate-now">
											...
											<input type="radio"  disabled="disabled" id="percent'.$value['stockresponseid'].'" data-deselect='.$value['stockresponseid'].' class="deselect percent testrad" name="desc'.$value['stockresponseid'].'"  onchange="descChanged('.$value['stockresponseid'].')" >
											<label for="percent'.$value['stockresponseid'].'" class="w-50 text-center testrad">%</label></li></ul></td>';
					$result_string1.='<td><div class="input-group"> <input type="text" name="discount_value[]" data_disc_value='.$value['stockresponseid'].' value="'.$value['discountvalue'].'"  id="enabledisc'.$value['stockresponseid'].'" class="enabledisc disctxt w-55" readonly></div>
					
										</td>';						
					$result_string1.='<td class=""><input type="hidden" class="form-control" data_gst_percent='.$value['stockresponseid'].' name="gst_percent[]" id="gst_percent'.$value['stockresponseid'].'" value="'.$value['gstrate'].'">
														<input type="text" name="discountext_value[]" id="disc_amount'.$value['stockresponseid'].'" class="add_discount text-right disctxt w-55" value="'.$value['discountvalueperquantity'].'"   readonly>
              ...
              // echo "<pre>"; print_r($in_sales); die;
                    ...
                    if(!empty($product)){
                        ...
                        foreach ($product as $key => $values) {  
                        	$fetch_array[1][] = array('productid'=>$values['productid'],'productname'=>$values['productname'],'salesauto_id'=>$in_sales[$values['productid']]['opsale_detailid']); 
                        } 
                    }
...
$headertable.='<tr ><td style="text-align:center;font-size:12px;" colspan="12" ><b>PHARMACY OP SALES</b></td></tr>';
...
$tbl1 = '<table cellspacing="0" cellpadding="1"  border="1" style="text-align:center;">
   ...
   <table>
      <tr>
   <td colspan="1" style="text-align:left;" ><b>MR Number</b></td><td colspan="6" style="text-align:left;"> : '.$saledata->mrnumber.'</td> 
   ...
   <td colspan="2" style="text-align:right;" ><b>Bill Date</b> : </td>  <td>'.date("d-m-Y",strtotime($saledata->invoicedate)).'</td>
    ...
    <tr>
    <td colspan="1" style="text-align:left;" ><b>DR Name</b>:</td><td colspan="6" style="text-align:left;"> : '.strtoupper($saledata->physicianname).'</td> 
...
public function actionPatientkey1($id) 
	...
	public function actionReturntabletbill($id)
	{
		...
		$salesreturn=Salesreturn::find()->where(['return_id'=>$id])->asArray()->one();
		
		$sales=Sales::find()->where(['opsaleid'=>$salesreturn['saleid']])->asArray()->one();
		
		
		$returndetails=Returndetail::find()->where(['return_id'=>$id])->asArray()->all();
...
$headertable.='<tr ><td style="text-align:center;font-size:12px;" colspan="12" ><b>PHARMA OP RETURN SLIP</b></td></tr>';
...
//$pdf->writeHTML($headertable, true, false, false, false, '');
		  ...
		  $s_val=$_POST['search']['value'];
			...
			if(!empty($Newpatient_da))
			{
				...
				foreach ($Newpatient_da as $key => $value) 
				{
					...
					$responce['data'][]=array("DT_RowId"=>$value['patientid'],"mrno"=>$value['mr_no'],"pname"=>$value['patientname'],"dob"=>date('d-m-Y',strtotime($value['dob'])),"age"=>$this->actionGetage($value['dob']),"rname"=>$value['par_relationname'],"mno"=>$value['pat_mobileno'],"city"=>$value['pat_city']);
					$i++;
				}
	...
	public function actionAjaxsinglefetch($id)
    {
		...
		if(!empty($id))
		{
			...
			$Newpatient=Newpatient::find()->select(['*'])
			->where(['temporary_blocked'=>'N'])->andWhere(['patientid'=>$id])->asArray()->one();
			
			
			$sub_visit=SubVisit::find()->where(['pat_id'=>$id])->orderBy(['sub_id'=>SORT_DESC])->asArray()->one();
			...
			if(!empty($Newpatient))
			{
				$physicianmaster=Physicianmaster::find()->select(['id'=>'id','physician_name'=>'physician_name'])->where(['id'=>$sub_visit['consultant_doctor']])->asArray()->one();
...
public function actionAjaxsinglefetch1($id)
	...
	public function actionBillhistory($fromdate,$todate,$pattype)
    {
		...
		if(isset($_POST['search'])){
		  $s_val=$_POST['search']['value'];
    		...
    		$sales=Sales::find()->select(['opsaleid'=>'opsaleid'])
			->where(['BETWEEN','invoicedate',$fromdate.' 00:00:00',$todate.' 23:59:59'])
			->andwhere(['sales_type'=>$pattype])
			->andwhere(['or',
				['like','billnumber',$s_val],
				['like','mrnumber',$s_val],
				['like','name',$s_val],
				['like','invoicedate',$s_val]]
				)
			->limit(100000)
			->orderBy(['opsaleid'=>SORT_DESC])->asArray()->all();
			...
			->limit($length)
			->offset($sfd)->orderBy(['opsaleid'=>SORT_DESC])->asArray()->all();
			...
			if(!empty($sales_da))
			{
				...
				$responce['draw']=$draw;
				$responce['recordsTotal']= $length;
				$responce['recordsFiltered']= count($sales);
				foreach ($sales_da as $key => $value) 
				{
					$responce['data'][]=array("DT_RowId"=>$value['opsaleid'],"billno"=>$value['billnumber'],"mrnum"=>$value['mrnumber'],"patname"=>$value['name'],"invdate"=>date('d-m-Y H:i:s',strtotime($value['invoicedate'])));
					$i++;
				}
...
public function actionBillhistoryfetch($id)
{
	...
	$stock_master_map=ArrayHelper::map($sales_detail,'opsale_detailid','stockid');
	$stock_master=Stockmaster::find()->where(['IN','stockid',$stock_master_map])->asArray()->all();
	$stock_master_index=ArrayHelper::index($stock_master,'stockid');
	
	$stock_response_map=ArrayHelper::map($sales_detail,'opsale_detailid','stockresponseid');
	$stock_response=Stockresponse::find()->where(['IN','stockresponseid',$stock_response_map])->asArray()->all();
	$stock_response_index=ArrayHelper::index($stock_response,'stockid');
	...
	//Insurance
	if(!empty($sales['insurancetype']))
	{
		$insurance=Insurance::find()->where(['insurance_typeid'=>$sales['insurancetype']])->asArray()->one();
		$ins_id=$insurance['insurance_typeid'];
		$ins_name=$insurance['insurance_type'];	
	}
	else 
	{
		$ins_id='';
...
$result_string1.='<tr  class="save_data_table" data-id='.$value['stockresponseid'].' id="table_del'.$value['stockresponseid'].'">';
$result_string1.='<td class="hide">'.$slno.'</td>';
$result_string1.='<td style="width:7%;"><div class="trunctext ">'.$product_index[$value['productid']]['productname']."/".$composition_index[$value['compositionid']]['composition_name'].'</div></td>';
$result_string1.='<td style="width:6%;">'.$value['batchnumber'].'</td>';
$result_string1.='<td style="width:8%;">'.date('d-m-Y',strtotime($value['expiredate'])).'</td>';
...
$result_string1.='<td class=""><input type="hidden" class="form-control" data_gst_percent='.$value['stockresponseid'].' name="gst_percent[]" id="gst_percent'.$value['stockresponseid'].'" value="'.$value['gstrate'].'">
		<input type="text" name="discountext_value[]" value="'.$value['discountvalueperquantity'].'" readonly="readonly" id="disc_amount'.$value['stockresponseid'].'" class="add_discount text-right disctxt w-55" readonly>
...
<input type="hidden" class="form-control total_amt_cal1 text-right" name="total_amt_cal1[]" data_total='.$value['stockresponseid'].' id="total_amount1'.$value['stockresponseid'].'" value="'.$value['total_price_perqty'].'">
</td>';
...
$ot[0]=$result_string1;
$ot[1]=$sales['tot_no_of_items'];
$ot[2]=$sales['tot_quantity'];
$ot[3]=$sales['overall_sub_total'];
$ot[4]=$sales['totalgstvalue'];
$ot[5]=$sales['overalldiscountpercent'];
$ot[6]=$sales['overalldiscountamount'];
$ot[7]=$sales['overalltotal'];
$ot[8]=$sales['round_off'];
$ot[9]=$sales['cash'];
$ot[10]=$sales['paid_amt'];
$ot[11]=$sales['due_amt'];

$ot[12]=$sales['mrnumber'];
$ot[13]=$sales['name'];
$ot[14]=$sales['physicianname'];
$ot[15]=date('d-m-Y',strtotime($sales['dob']));
$ot[16]=$sales['address'];
$ot[17]=$sales['phonenumber'];
$ot[18]=$ins_id;
$ot[19]=$ins_name;
			...
			if(!empty($Sale_detail))
			{
				...
				foreach ($Sale_detail as $key => $value)
				{
					...
					$result_string1.='<td><ul class="donate-now">
											...
											<label for="percent'.$value['stockresponseid'].'" class="w-50 text-center testrad">%</label></li></ul></td>';
					$result_string1.='<td><div class="input-group"> <input type="text" name="discount_value[]" data_disc_value='.$value['stockresponseid'].' id="enabledisc'.$value['stockresponseid'].'" class="enabledisc disctxt w-55" readonly></div>
					
										</td>';						
					$result_string1.='<td class=""><input type="hidden" class="form-control" data_gst_percent='.$value['stockresponseid'].' name="gst_percent[]" id="gst_percent'.$value['stockresponseid'].'" value="'.$value['gstrate'].'">
														<input type="text" name="discountext_value[]" id="disc_amount'.$value['stockresponseid'].'" class="add_discount text-right disctxt w-55" readonly>
